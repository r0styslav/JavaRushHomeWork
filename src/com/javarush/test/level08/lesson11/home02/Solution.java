...
public class Solution
{
    ...
    {
        //напишите тут ваш код
        boolean catsHaveNext = true;
        boolean dogsHaveNext = true;
        HashSet<Object> joinSet = new HashSet<>();
        Iterator<Cat> catIterator = cats.iterator();
        Iterator<Dog> dogIterator = dogs.iterator();
        while (catsHaveNext || dogsHaveNext) {
            if (catIterator.hasNext())
                joinSet.add(catIterator.next());
            else
                catsHaveNext = false;
            if (dogIterator.hasNext())
                joinSet.add(dogIterator.next());
            else
                dogsHaveNext = false;
        }
